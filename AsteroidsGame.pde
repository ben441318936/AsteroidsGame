...
public void draw() 
{
  if (gamestart==true)
  {
    ...
    if (HMS_Euphoria.getAccelerating()==true) {HMS_Euphoria.accelerate(0.05);}
...
{   
  ...
  {      
    if (myDirectionX>=2.5) {myDirectionX=2.5;}
    if (myDirectionX<=-2.5) {myDirectionX=-2.5;}
    if (myDirectionY>=2.5) {myDirectionY=2.5;}
    if (myDirectionY<=-2.5) {myDirectionY=-2.5;}
...
class Asteroid extends Floater
{
  ...
  {
    myColor=colour(90, 90, 90);
    myDirectionX=(Math.random()*2-0.95)*k;
    myDirectionY=(Math.random()*2-0.95)*k;
    myPointDirection=0;
    double x=Math.random();
    if (x<=0.5) {myDirectionOfRot=1;}
    if (x>0.5) {myDirectionOfRot=-1;}
